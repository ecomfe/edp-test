...
exports.build = function (emitter) {
    ...
    var contextTpl = fs.readFileSync(
        path.resolve(clientDir, 'context.html'),
        { encoding: 'utf-8' }
    );

    var debugTpl = fs.readFileSync(
        path.resolve(clientDir, 'debug.html'),
        { encoding: 'utf-8' }
    );
    ...
    config.requireConfig.urlArgs = 'd=' + (+new Date()).toString(36);
    ...
    cached.runner = fs.readFileSync(path.resolve(clientDir, 'runner.html'), 'utf-8');
...
exports.istanbul = function () {
    return function (context) {
        ...
        context.content = new istanbul.Instrumenter().instrumentSync(content, src);
    };
};
