...
function buildCases(file) {
    ...
    code += file.cases.map(function (item) {
        return 'suite.add(\'' + item.name + '\', function () {\n' + item.js.join('\n') + '\n});\n';
    }).join('');
...
function buildTPL(tpl, files, frameworks, requireConfig) {
    return tpl
...
exports.bench = function (emitter, args) {
    log.info('Build runner...');
    ...
    filesKeys.forEach(function (key) {
        var file = files[key];
        var tpl = buildTPL(contextTpl, file, frameworks, requireConfig);
        cached['bm-context-' + key] = tpl;
        cached['bm-debug-' + key] = buildTPL(debugTpl, file, frameworks, requireConfig);
    });
